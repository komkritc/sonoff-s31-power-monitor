...
volatile bool espNowRequestPending = false;
...
void handleESPNowRequest() {
  ...
  if (!espNowRequestPending) return;

  espNowRequestPending = false;

  Packet reply;
  reply.seq = requestSeq;
  reply.timestamp = micros();
  ...
  if (!success) {
    strcpy(reply.msg, "{\"err\":1}");
    esp_now_send(requestMAC, (uint8_t *)&reply, sizeof(reply));
    return;
  }
...
void onReceive(uint8_t *mac, uint8_t *data, uint8_t len) {
  ...
  if (strcmp(incoming.msg, "get_measure") == 0) {
    ...
    memcpy(requestMAC, mac, 6);
    requestSeq = incoming.seq;

    espNowRequestPending = true;   // ✅ JUST SET FLAG
  }
}
