 ...
 *              and comprehensive pump protection features including intelligent overload
 *              protection with inrush current handling.
 * 
 * ================================================================================================
 ...
 *   - Configurable inrush tolerance period
 *   - Configurable cooldown after overload
 * ✓ Sensor failure handling with 4 configurable strategies
 * ✓ Real-time power monitoring (Watts, Volts, Amps, Power Factor)
 * ✓ Pump statistics tracking (runtime, cycles, energy consumption, overload events)
 * ✓ OTA updates support
 ...
 * 
 * ================================================================================================
 * @section     OVERLOAD PROTECTION WITH INRUSH HANDLING
 * ================================================================================================
 * 
 * PROBLEM: Pump motors draw 2-7x normal current for 100-2000ms at startup (inrush current).
 *          Without special handling, this would falsely trigger overload protection.
 * 
 * SOLUTION: Intelligent overload protection that:
 *   1. Detects when pump starts and enters "inrush tolerance period"
 *   2. Ignores high current during this period (logs but doesn't trigger)
 *   3. Only triggers protection for SUSTAINED overload after normal operation
 *   4. Implements cooldown period to prevent rapid cycling after overload
 * 
 * PROTECTION TYPES:
 *   1. INRUSH TOLERANCE: Allows 2-7x power for configurable time (default 2000ms)
 *   2. SUSTAINED OVERLOAD: Triggers when power exceeds limit AFTER inrush period
 *   3. DRY RUN: Triggers when power is too low for too long (auto mode only)
 *   4. COOLDOWN: Prevents restart for configurable time after overload
 * 
 * ================================================================================================
 */
...
// @brief       Clean, minimal interface for everyday users
// ================================================================================================
...
<body>
    <div class="container">
        <div class="card">
            ...
            <div class="eng-badge">⚙️ ENGINEERING MODE - Overload protection with inrush handling works in BOTH Auto AND Manual modes</div>
    ...
    <script>
        ...
        async function loadPumpSettings(){const cfg=await fetchConfig();document.getElementById('lowThreshold').value=cfg.low_threshold||30;document.getElementById('highThreshold').value=cfg.high_threshold||80;document.getElementById('minPower').value=cfg.min_power||5;document.getElementById('dryRunProtection').value=cfg.dry_run_protection||300;document.getElementById('loadProtectionToggle').checked=cfg.pump_load_protection_enabled!==false;document.getElementById('maxPower').value=cfg.max_power_threshold||500;document.getElementById('inrushTolerance').value=cfg.inrush_tolerance_ms||2000;document.getElementById('cooldownPeriod').value=cfg.overload_cooldown_seconds||30;}
...
// @brief       Protects pump from sustained overload while allowing normal inrush current
// ================================================================================================
 ...
 * 
 * @details This function implements intelligent overload protection that:
 *          1. Detects when pump starts and enters "inrush tolerance period"
 *          2. Ignores high current during inrush (prevents false trips)
 *          3. Only triggers protection for SUSTAINED overload after normal operation
 *          4. Implements cooldown period to prevent rapid cycling
 * 
 * Why this is necessary:
 * - Motor pumps draw 2-7x normal current for 100-2000ms at startup
 * - Without this handling, normal startup would falsely trigger protection
 * - With this handling, protection only triggers when pump runs too hot for too long
 */
bool checkOverloadProtection(float currentPower, bool currentState) {
  ...
  if (currentState && !overload.lastRelayState) {
    // Pump just started! Record start time for inrush tolerance period
  ...
  if (overload.waitingForStableOperation && currentState) {
    ...
    if (runtime < config.inrush_tolerance_ms) {
      // Still in inrush period - ignore overload detection
      // Log unusually high current (for debugging) but don't trigger protection
      ...
      return true;  // Allow pump to run during inrush (no matter how high the current)
    } else {
      // Inrush period ended, now monitoring normally
  ...
  if (overload.cooldownUntil > now) {
    ...
    if (currentState) {
      // Force pump off during cooldown period
      s31.setRelay(false);
      if (remaining > 0 && remaining % 5 == 0) {  // Log every 5 seconds to avoid spam
        addFailureLogEntry("⏱️ Cooldown active: " + String(remaining) + 
                           "s remaining - Pump blocked from starting");
      }
    }
    return false;  // Pump not allowed to run during cooldown
  }
  
  // Reset protection flag if cooldown expired
  ...
  // ==============================================================================================
  // SUSTAINED OVERLOAD DETECTION (only after inrush period)
  // ==============================================================================================
  if (config.pump_load_protection_enabled && currentState && currentPower > config.max_power_threshold) {
    // Double-check: Make sure we're not still in the inrush period
    unsigned long runtime = now - overload.pumpStartTime;
    
    if (runtime < config.inrush_tolerance_ms) {
      // Still in inrush - this is normal, don't trigger protection
      addFailureLogEntry("📈 Inrush current detected: " + String(currentPower, 1) + 
                         "W (normal for startup) - Protection bypassed");
      return true;
    }
    
    // Legitimate sustained overload detected (running for a while AND power too high)
    if (!overload.active) {
      ...
      // Dramatic logging for sustained overload
      addFailureLogEntry("╔════════════════════════════════════════════════════════════════╗");
  ...
  return true;  // All good - pump can run normally
}

// ================================================================================================
// @section     PUMP CONTROL LOGIC (WITH INTELLIGENT OVERLOAD)
// ================================================================================================

/**
 * @brief   Main pump control routine with intelligent overload protection
 * @note    Overload protection works in BOTH Auto and Manual modes
 * @note    Inrush tolerance prevents false trips during motor startup
 */
void controlPump() {
  updateSensorHealth();
  updatePowerQuality();
  ...
  // ==============================================================================================
  // ★ INTELLIGENT OVERLOAD PROTECTION - CHECKED FIRST ★
  // ==============================================================================================
  // This runs REGARDLESS of auto/manual mode to ensure maximum safety
  ...
  // Determine desired pump state based on water level
  if (!dataValid || sensorIsDead) {
    ...
    if (currentWaterLevel <= config.low_threshold) {
      ...
      shouldPumpOn = currentState;  // Maintain current state in deadband
    }
  }
  ...
  if (currentState && currentPower < config.min_power_threshold && !sensorIsDead) {
    ...
    // Ensure we're past the inrush period before checking dry run
...
/**
 * @brief   Adds a formatted entry to the ESP-NOW message log
 */
...
/**
 * @brief   Generates HTML table of ESP-NOW message log
 */
...
/**
 * @brief   ESP-NOW receive callback - parses incoming sensor data
 */
void OnDataRecv(uint8_t *mac, uint8_t *data, uint8_t len) {
  ...
  // Parse JSON-like format: {"d":351.9,"l":0.0,"v":0.0,"b":3.80}
...
/**
 * @brief   Initializes ESP-NOW communication with configured peer
 */
void initEspNow() {
...
/**
 * @brief   Sends a command string via ESP-NOW
 */
...
/**
 * @brief   Periodically requests sensor data (called from loop)
 */
...
/**
 * @brief   Manually requests sensor data (triggered by UI button)
 */
...
/**
 * @brief   Configures device as WiFi Access Point
 */
void setupAPMode() {
...
/**
 * @brief   Initializes Over-The-Air firmware updates
 */
...
/**
 * @brief   Configures all HTTP endpoints for the web server
 */
void setupWebServer() {
  // UI Pages
  ...
  server.on("/data", HTTP_GET, []() {
    ...
    // Overload protection status - FIXED: No max() function with mismatched types
    doc["overloadProtectionActive"] = overload.active;
    doc["inInrushPeriod"] = overload.waitingForStableOperation;
    
    // Calculate inrush remaining time safely without max()
...
void loop() {
  ...
  // Update Sonoff S31 power readings
  ...
  // Request sensor data via ESP-NOW
  requestSensorData();
  
  // Run pump control logic (includes overload protection)
  controlPump();
  
  // Update mDNS announcements
